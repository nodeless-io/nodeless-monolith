...
const ViewWithdrawTransactionModal = () => {
  const {
      ...
      title={
        <Box sx={{ marginTop: "10px" }}>
          ...
          >
            ...
            >
              <span>#{currentTransaction?.transactable?.uuid}</span>
            </Typography>
          </Box>

          <Box sx={styles.header}>
            <Typography sx={styles.amount}>
              {formatter(currentTransaction?.transactable?.amount)}{" "}
              <span>SATS</span>
            </Typography>

            <Box sx={{ marginTop: "-10px" }}>
              <WithdrawStatusBadge
                status={currentTransaction?.transactable?.status}
              />
            </Box>
          </Box>
        </Box>
        ...
        <Box sx={styles.section}>
          ...
          {currentTransaction?.is_fee && (
            <Grid container spacing={2} sx={styles.details}>
              ...
              <Grid item xs={12} md={9} sx={styles.rightGrid}>
                <Typography sx={styles.detailsItemValue}>Fees</Typography>
              </Grid>
            </Grid>
          )}
          ...
          <Grid container spacing={2} sx={styles.details}>
            ...
            <Grid item xs={12} md={9} sx={styles.rightGrid}>
              <Typography sx={styles.detailsItemValue}>
                {currentTransaction?.transactable?.type === "lightning"
                  ? "Lightning"
                  : "On Chain"}
              </Typography>
            </Grid>
          ...
          {currentTransaction?.transactable?.type === "lightning" && (
            <Grid container spacing={2} sx={styles.details}>
              ...
              <Grid item xs={12} md={9} sx={styles.rightGrid}>
                <Typography sx={styles.detailsItemValue}>
                  {currentTransaction?.transactable?.lightning_address}
                </Typography>
              </Grid>
            </Grid>
          )}
            ...
            <Grid container spacing={2} sx={styles.details}>
              ...
              <Grid item xs={12} md={9} sx={styles.rightGrid}>
                <a
                  ...
                  href={`${MEMPOOL_ADDRESS_URL}/${currentTransaction?.transactable?.onchain_address}`}
                >
                  {currentTransaction?.transactable?.onchain_address}
                </a>
              </Grid>
            </Grid>
          )}
          ...
          {currentTransaction?.transactable?.type === "lightning" && (
            <Grid container spacing={2} sx={styles.details}>
              ...
              <Grid
                ...
                onClick={() =>
                  copyToClipboard(
                    currentTransaction?.transactable?.lightning_payment_hash,
          ...
          {currentTransaction?.transactable?.type === "lightning" && (
            <Grid container spacing={2} sx={styles.details}>
              ...
              >
                <Typography sx={styles.detailsItemValue}>
                  {currentTransaction?.transactable?.lightning_payment_preimage}
                </Typography>
              </Grid>
            </Grid>
          )}
            ...
            <Grid container spacing={2} sx={styles.details}>
              ...
              <Grid item xs={12} md={9} sx={styles.rightGrid}>
                <a
                  ...
                  href={`${MEMPOOL_TX_URL}/${currentTransaction?.transactable?.onchain_tx}`}
                >
                  {currentTransaction?.transactable?.onchain_tx}
                </a>
              </Grid>
            </Grid>
          )}
            ...
            <Grid container spacing={2} sx={styles.details}>
              ...
              <Grid
                ...
                onClick={() =>
                  copyToClipboard(
                    JSON.stringify(
                      currentTransaction?.transactable?.lightning_payment_route
                    ),
...
export default ViewWithdrawTransactionModal;
