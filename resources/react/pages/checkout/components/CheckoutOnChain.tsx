...
const CheckoutOnChain = () => {
  const { checkout } = React.useContext(CheckoutContext);

  return (
    <Box sx={styles.content}>
      <div>
        <a href={checkout?.onchain_link}>
          <img
            src={checkout?.onchain_qr_code}
            width={250}
            height={250}
      ...
      <LinkButton
        ...
        link={checkout?.onchain_link}
      />

      <Box sx={styles.inputBox}>
        <Typography sx={styles.amountBox}>
          {shortenString(checkout?.store_invoice.bitcoin_address.address, 40)}
        </Typography>
...
export default CheckoutOnChain;
