...
const ForgotPasswordPage = () => {
  ...
  const forgotPassword = async (payload) => {
    return await useFetch(`/forgot-password`, payload, "POST");
  };

  const { isLoading, mutateAsync } = useMutation(forgotPassword, {
    onSuccess: () => {
      message.success(
        "A link to reset your password has been sent to the provided email address"
      );
    },
    onError: (error: any) => {
      message.error(
      ...
      <Box sx={styles.content}>
        <Box>
          <Box sx={styles.subHeader}>
            <Typography sx={styles.headerText}>Forgot Password?</Typography>
