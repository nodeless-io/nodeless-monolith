...
export const WithdrawTransactionsContext = createContext<{
  transactions: any[];
  loading: boolean;
  page: number;
...
export const WithdrawTransactionsContextProvider = (props) => {
  ...
  const { data, isLoading, refetch } = useQuery(
    ["withdraw-transactions", page],
    async () => {
      return await useFetch(
        `/transaction/withdrawals?isFee=true&page=${page + 1}`
      );
    }
  );
  ...
  return (
    <WithdrawTransactionsContext.Provider value={value}>
      {props.children}
    </WithdrawTransactionsContext.Provider>
  );
};
