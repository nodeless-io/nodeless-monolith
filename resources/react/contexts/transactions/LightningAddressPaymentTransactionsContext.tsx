...
export const LightningAddressPaymentTransactionsContext = createContext<{
  transactions: any[];
  loading: boolean;
  page: number;
...
export const LightningAddressPaymentTransactionsContextProvider = (props) => {
  ...
  const { data, isLoading, refetch } = useQuery(
    ...
    async () => {
      return await useFetch(
        `/transaction/lightning-address-payments?isFee=true&page=${page + 1}`
      );
    }
  );
  ...
  return (
    <LightningAddressPaymentTransactionsContext.Provider value={value}>
      {props.children}
    </LightningAddressPaymentTransactionsContext.Provider>
  );
};
